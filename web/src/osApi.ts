...
export const osApi = {
  ...
  // waitMs > 0 asks the daemon to hold the request until the process writes (or the wait
  // elapses), so terminal output arrives at network latency instead of on a poll boundary
  readProcessOutput: async (processId: OsId, after = 0, waitMs = 0) => {
    const raw = await api('GET', `/os/processes/${processId}/output?after=${after}${waitMs > 0 ? `&wait=${waitMs}` : ''}`)
