...
export function App({ authorityMode = 'local-owner', onAuthorityChanged }: {
  ...
  // This is the local board only — personal, one machine, no org. The shared
  // cloud workspace is a separate bundle with its own entry (web/src/cloud-main.tsx,
  // built by `npm run build:cloud`), so no build-time env var can turn a personal
  // board into the cloud sign-in page. Agents still reach the org through the
  // daemon's `orchestra org join` device token, which never involves this bundle.
