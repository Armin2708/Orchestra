...
`031-operations-recovery-foundation`, which must follow root-owned migration
`030-device-sessions`. Its transactional installer creates all seven owned tables/indexes only
...
1. Add migration `031-operations-recovery-foundation` after `030-device-sessions`, call
