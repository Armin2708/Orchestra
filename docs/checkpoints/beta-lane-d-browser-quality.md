...
- Browser evidence schema v8 defines startup as valid owner-token submission through authenticated
  ...
  including requests still in flight at live readiness. Paths and request IDs are retained only as
...
- Fixture creation is paced before every request against the production fixed-window families:
  request 800/200, command 200/40 and provider 50/10. Atomic reservations keep concurrent batches at
  20 or fewer in flight; a full family waits from the latest completed response, and Chromium cannot
  start until 60 seconds after the final fixture response. The exact 275 request, 273 command and one
  provider starts are retained as aggregate-only provenance. A separate run-scoped CDP pacer then
  counts every same-origin `/api/` start across all viewport lifecycles and waits at the 800-request
  ceiling with a 200-request reserve. Any 429 or lifecycle above 200 starts fails closed; evidence
  never retains URLs, paths, request IDs or response bodies.
