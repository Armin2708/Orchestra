...
- A focused, replay-safe schema-v2 migration with update-immutable but retention-deletable evidence,
...
- Privacy-safe exploration accounting: callers supply a resource identity, but only a per-install
  keyed HMAC is persisted or returned; legacy unkeyed digests are re-keyed on schema-v2 upgrade.
...
- Canonical provider/session/job attribution, conservative billing derivation from retained
  ...
  artifact-attested benchmark metrics. Unattested operator claims fail closed.
...
- Focused backend/service/API/UI-contract suite: 3 files, 21 tests (parallel and one-worker).
