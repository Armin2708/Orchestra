...
halves on the captured BASE-008 source: **50.3% fewer injected tokens per session,
...
|---|---|---|---|
| session_start (rules + board dump) | 765 | 312 | 59.2% |
| user_prompt_submit (nudges) | 38 | 38 | 0% |
| post_tool_use (messages + nudges) | 51 | 51 | 0% |
| stop (block reasons, 4 turn-ends) | 50 | 50 | 0% |
| **total** | **903** | **449** | **50.3%** |

Measured locally on 2026-07-28 at exact clean commit
`51b168d96becccd4aa3506dec9e80fcebda43ed7` by `node scripts/ab-token-diet.mjs`
and retained in `docs/agent-os-current-baseline.json`. Per-event token counts are
rounded independently; the total is calculated from the full concatenated
character count.
...
| output-discipline block is present | ✓ | ✓ |
...
- The A/B suite (`test/token-diet-ab.test.ts`) runs the verbose arm through this
  ...
  Measured on the BASE-008 source: verbose arm 903 tok vs compact 449 tok
  per session.
...
board messages and card descs carry deltas only, final summaries use at most three
Delivered/Evidence/Remaining bullets, and comments appear only where code cannot say it.

**Input cost of the block: 130 chars ≈ 33 tok per session** (CI-asserted <60 in
...
stay green. Its 130 characters are included identically in both totals, so the block cannot
inflate the measured #35 savings.
...
Net: the block pays for its 33-token input cost as soon as a session produces ~95
