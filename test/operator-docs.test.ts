...
describe('operator onboarding documentation', () => {
  it('documents every owned PKG boundary without claiming unfinished support', () => {
    ...
    expect(read('data-recovery.md')).toContain('scripts/backup-orchestra-state.sh')
    expect(read('data-recovery.md')).toContain('set -euo pipefail')
    expect(read('data-recovery.md')).toMatch(/HOME.*ORCHESTRA_HOME.*absolute/s)
    expect(read('data-recovery.md')).toMatch(/integrity_check.*exactly `ok`/s)
    expect(read('data-recovery.md')).toMatch(/mode `600`/)
    ...
    expect(read('telemetry-support.md')).toMatch(/independent strict byte verifier/)
    expect(read('telemetry-support.md')).toMatch(/No issue-tracker, email, upload or publication transport is registered/)
    expect(read('support-preview.md')).toContain('orchestra ops support-case')
    expect(read('support-preview.md')).toMatch(/Nothing is uploaded or submitted/)
    expect(read('operator-preview.md')).toMatch(/`onboarding\.json`/)
    expect(read('operator-preview.md')).toMatch(/`lifecycle-demo-locks\/\*\.lock`/)
