...
describe('packaging documentation truthfulness', () => {
  it('documents scoped pairing without reviving master-token bootstrap', () => {
    ...
    expect(readme).toContain('signs each phone in with the owner password')
    expect(readme).toContain('scoped, revocable DeviceSession per device')
    ...
    expect(cli).toContain('sign in with your password')
    expect(cli).toContain('pairing tickets are deprecated')
