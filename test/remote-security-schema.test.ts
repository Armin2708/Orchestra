...
describe('remote security migration attestation', () => {
  it('replays migration 030 after marker loss without changing the attested object set', () => {
    ...
    db.prepare("DELETE FROM os_schema_migrations WHERE id='030-device-sessions'").run()
    applyAgentOsMigrations(db)
    expect(db.prepare("SELECT id FROM os_schema_migrations WHERE id='030-device-sessions'").get())
      .toEqual({ id: '030-device-sessions' })
