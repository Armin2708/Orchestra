  ...
  it('permits only reviewed commit-scoped redaction fixtures', () => {
    ...
    for (const fingerprint of fingerprints) {
      expect(fingerprint).toMatch(
        /^[0-9a-f]{40}:test\/(?:agent-home-domain|agent-os-migrations|projected-text-redaction|organization-coordination|command-idempotency-coverage)\.test\.ts:(?:private-key|generic-api-key):\d+$/,
      )
    }
  })
})
