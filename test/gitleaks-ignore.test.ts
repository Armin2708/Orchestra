  ...
  it('permits only reviewed commit-scoped test fixtures', () => {
    ...
    for (const fingerprint of fingerprints) {
      expect(fingerprint).toMatch(
        /^[0-9a-f]{40}:test\/(?:agent-home-domain|agent-os-migrations|projected-text-redaction)\.test\.ts:private-key:\d+$|^[0-9a-f]{40}:test\/(?:session-tool-routes|organization-coordination|command-idempotency-coverage)\.test\.ts:generic-api-key:\d+$/,
      )
    }
  })
})
