...
describe('Agent OS migrations', () => {
  ...
  it('creates the kernel schema exactly once across repeated opens', () => {
    ...
    expect((first.prepare('SELECT COUNT(*) AS count FROM os_schema_migrations').get() as any).count).toBe(31)
    first.close()
    ...
    expect((second.prepare('SELECT COUNT(*) AS count FROM os_schema_migrations').get() as any).count).toBe(31)
    const migrationIds = (second.prepare(
      'SELECT id FROM os_schema_migrations ORDER BY rowid',
    ).all() as Array<{ id: string }>).map((row) => row.id)
    expect(migrationIds.slice(-17)).toEqual([
      ...
      '030-device-sessions',
      '031-operations-recovery-foundation',
    ])
    expect(migrationIds.at(-1))
      .toBe('031-operations-recovery-foundation')
  ...
  it('applies migration 015 forward and fails closed on displaced or ambiguous action state', () => {
    ...
    expect((db.prepare('SELECT COUNT(*) AS count FROM os_schema_migrations')
      .get() as { count: number }).count).toBe(31)
  ...
  it('upgrades a migration-003 database without rewriting legacy contract meaning', () => {
    ...
    expect((db.prepare('SELECT COUNT(*) AS count FROM os_schema_migrations').get() as any).count).toBe(31)
