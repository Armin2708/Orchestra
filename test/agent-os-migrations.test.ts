...
describe('Agent OS migrations', () => {
  ...
  it('creates the kernel schema exactly once across repeated opens', () => {
    ...
    expect((first.prepare('SELECT COUNT(*) AS count FROM os_schema_migrations').get() as any).count).toBe(28)
    first.close()
    ...
    expect((second.prepare('SELECT COUNT(*) AS count FROM os_schema_migrations').get() as any).count).toBe(28)
    ...
    expect(migrationIds.at(-1))
      .toBe('028-agent-organization-coordination')
  ...
  it('applies migration 015 forward and fails closed on displaced or ambiguous action state', () => {
    ...
    expect((db.prepare('SELECT COUNT(*) AS count FROM os_schema_migrations')
      .get() as { count: number }).count).toBe(28)
  ...
  it('upgrades a migration-003 database without rewriting legacy contract meaning', () => {
    ...
    expect((db.prepare('SELECT COUNT(*) AS count FROM os_schema_migrations').get() as any).count).toBe(28)
