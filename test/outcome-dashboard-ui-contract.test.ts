...
describe('outcome dashboard UI contract', () => {
  ...
  it('uses memory-authenticated polling and retains streaming for tokenless loopback daemons', () => {
    ...
    expect(setup).toContain('authenticatedPolling ? undefined : window.setTimeout(requestRefresh, 0)')
    expect(setup.indexOf('window.setTimeout(requestRefresh, 0)'))
      .toBeLessThan(setup.indexOf('if (authenticatedPolling) requestRefresh()'))
