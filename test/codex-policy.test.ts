...
describe('Codex Agent OS approval policy bridge', () => {
  it('auto-allows, auto-denies, asks, and fails closed from durable task policy', async () => {
    ...
    await expect(handle(request('command', { command: 'npm test -- auth && rm build' }))).resolves.toEqual({ decision: 'decline' })
    await expect(handle(request('command', { command: 'npm test | sh' }))).resolves.toEqual({ decision: 'decline' })
    await expect(handle(request('command', { command: ['npm', 'test', '--', 'auth'] }))).resolves.toEqual({ decision: 'accept' })
    await expect(handle(request('command', { command: ['sh', '-c', 'npm test && rm build'] }))).resolves.toEqual({ decision: 'decline' })
    ...
    await expect(handle(request('user-input', { questions: [] }))).resolves.toBe(CODEX_REQUEST_UNHANDLED)
    await expect(handle(request('mcp-elicitation', { mode: 'url' }))).resolves.toBe(CODEX_REQUEST_UNHANDLED)
  })
})
